...
void mouse_move_task(void *pvParameters)
{
    ...
    for (;;) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            xSemaphoreGive(s_local_param.mouse_mutex);
            for (int j = 0; j < 100; j++) {
                leftButton = gpio_get_level(LEFT_BUTTON);
                rightButton = gpio_get_level(RIGHT_BUTTON);
                mpu.getAccdata_scale(10,&acc);
        
                x = acc.y * -1;
                y = acc.z;
                if(leftButton){
                    send_mouse_report(1, x, y, 0);
                }else if(rightButton){
                    send_mouse_report(2,x,y,0);
                }else{
                    send_mouse_report(0,x,y,0);
                }
...
extern "C" void app_main()
{
    ...
    esp_bt_dev_set_device_name("Motion Mouse");
